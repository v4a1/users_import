...
def render_tver(data, columns_name):
    day = datetime.now().strftime("%d.%m.%Y")
    items = [{"email": "Почта", "Work email": day, "Accounts": day}]
    data_keys = list(data)
    data_keys.sort()
    for user in data_keys:
        item = {}
